...
SocketService.io.on('connection', (socket) => {
  socket.on(PREFIX + 'join', async (channelId) => {
    ...
    try {
      ...
      await socket.emit(channelService.ROOM_PREFIX + 'join:error', {message: err.message});
    }
  });
  socket.on(PREFIX + 'leave', async (channelId) => {
    ...
    try {
      ...
      await socket.emit(channelService.ROOM_PREFIX + 'leave:error', {message: err.message});
    }
  });
});
